...
type ServerCmd struct {
	...
	DatabaseURL string `env:"DATABASE_URL" required:"" help:"URL for connecting to the PostgreSQL database."`
	WorkerCount int    `env:"WORKER_COUNT" default:"50" help:"Number of workers to run in the worker pool."`
	OTLP        struct {
...
func (c *ServerCmd) Run(ctx context.Context, g *Globals) error {
	...
	worker, err := worker.NewPool(ctx, "app.worker", db, g.Writer, c.WorkerCount)
	if err != nil {
