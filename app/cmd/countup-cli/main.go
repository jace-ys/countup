...
func main() {
	var (
		hostF = flag.String("host", "dev-http", "Server host (valid values: dev-http, dev-grpc)")
	...
	)
	{
		...
		if addr == "" {
			...
			default:
				fmt.Fprintf(os.Stderr, "invalid host argument: %q (valid hosts: dev-http|dev-grpc)\n", *hostF)
				os.Exit(1)
			}
		}
		timeout = *timeoutF
...
func usage() {
	fmt.Fprintf(os.Stderr, `%s is a command line client for the countup API.

Usage:
    %s [-host HOST][-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -host HOST:  server host (dev-http). valid values: dev-http, dev-grpc
