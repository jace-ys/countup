...
func (s *Service) GetCounterInfo(ctx context.Context) (*CounterInfo, error) {
...
func (s *Service) RequestIncrement(ctx context.Context, user string) error {
	...
	var enqueuePostCommitHook worker.EnqueuePostCommitHook
	...
	if existing > 0 {
		...
		enqueuePostCommitHook, err = s.workers.EnqueueTx(ctx, tx, FinalizeIncrementJobArgs{
		...
		err = s.store.UpdateCounterFinalizeTime(ctx, tx, pgtype.Timestamptz{
			Time:  finalizeAt,
			Valid: true,
		})
		if err != nil {
	...
	if enqueuePostCommitHook != nil {
		enqueuePostCommitHook()
	}
